...
const PROVIDER: ProviderName = "binance_global"
const BASE_URL = "https://api.binance.com"
const EXCHANGE_INFO_CACHE_KEY = "binance_global:exchange_info"
...
export const listTradableUsdtBases = async (): Promise<UsdtBaseCatalogEntry[]> => {
  ...
  return info.symbols
    ...
    .map((entry) => ({
      ...
      source: "binance_global" as const,
    }))
...
export const fetchWeeklyOhlcv = async (
  ...
  try {
    ...
    const message =
      error instanceof ProviderError ? error.message : "Failed to fetch Binance Global OHLCV"
...
export const resetBinanceGlobalCache = (): void => {
